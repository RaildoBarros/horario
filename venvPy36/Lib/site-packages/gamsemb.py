...
class ECSymbol(object):
    ...
    def __init__(self, ecDB, symPtr, keyType, keyFormat, valueFormat, recordFormat, mergeType=MergeType.DEFAULT, domCheck=True):
        self._ecDB = ecDB
        self._symPtr = symPtr
...
class ECGAMSDatabase(object):
    ...
    def _setSet(self, ecSymbol, data):
        ...
        if not isinstance(firstRec, tuple):
            ...
            if isinstance(firstRec[0], tuple):
                parseKeys = lambda rec: list(rec[0])
                valueIdx = 1
            else:
                parseKeys = lambda rec: list(rec[0:ecSymbol._dim])
    ...
    def set(self, symbolName, data, mergeType=MergeType.DEFAULT, domCheck=True):
        ...
        self._modSymList[ecSymbol.getSymbolNumber()] = (merge_type_to_int(ecSymbol._mergeType), ecSymbol._domCheck)
